<?php 
	...
	class Transportes extends Controllers{
		...
		{
			$data['page_id'] = 3;
			$data['page_tag'] = "Transportes";
			$data['page_name'] = "transportes";
			$data['page_title'] = "Transportes ";
		...
		public function setTransporte(){
			...
			{
				//Crear
				$request_transporte = $this->model->insertTransporte($strTransporte, $strDescripcion,
				$strClase, $strTipo, $strDisponibilidad,$intPrecio, $strTelefono,$intStatus,$imgImagen);
				$option = 1;
			}else{
				//Actualizar
				if($nombre_foto == ''){
