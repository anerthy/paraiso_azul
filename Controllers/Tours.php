	...
	class Tours extends Controllers{
		...
		public function getTours()
		{
			...
			for ($i=0; $i < count($arrData); $i++) {
				...
				<button class="btn btn-primary btn-sm btnEditTour" onClick="fntEditTour('.$arrData[$i]['id_tour'].')" title="Editar"><i class="fas fa-pencil-alt"></i></button>
				<button class="btn btn-danger btn-sm btnDelTour" onClick="fntDelTour('.$arrData[$i]['id_tour'].')" title="Eliminar"><i class="far fa-trash-alt"></i></button>
				<button class="btn btn-info btn-sm" onClick="fntViewInfo('.$arrData[$i]['id_tour'].')" title="Ver tour"><i class="far fa-eye"></i></button>
				</div>';
			}
		...
		public function setTour(){
			...
			if($intId_tour == 0)
			{
				...
				$request_tour = $this->model->insertTour($strNombre_tour,
                $strDescripcion,
                $strServicio,
                $strActividad,
                $strAlimentacion,
